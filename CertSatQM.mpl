...
# encoded as a list of polynomials
    ...
    local a, b, c1, c2, _gamma;
        ...
        for i from 1 to nops(simpl_roots[1, 1]) do
            ...
            _temp := zeroPO(nat);
            updatePONatEntry(_temp, 0, a - c1);
            updatePONatEntry(_temp, 1,      1);
        ...
        for i from 2 to size - 1 do
            ...
            while j <= k do
                ...
                _temp := zeroPO(nat);
                updatePONatEntry(_temp,       0, (x-c1)*(x-c2) - _gamma*(x-a)*(x-b));
                updatePONatEntry(_temp, 2^(i-1),                             _gamma);
                output := prodPO(output, _temp, nat, x);
                j := j + 1;
                k := k - 1;
            end do;
        ...
        for i from 1 to nops(simpl_roots[size, 1]) do
            ...
            _temp := zeroPO(nat);
            updatePONatEntry(_temp,          0, c2 - b);
            updatePONatEntry(_temp, 2^(size-1),      1);
            output := prodPO(output, _temp, nat, x);
        end do;
    ...
    local output := zeroQM(nat);
        if a = b then
            updateQMNatEntry(output,    x - a, (x-a-1)^2/4);
            updateQMNatEntry(output, -(x - a), (x-a+1)^2/4);
        elif a < b then
            updateQMNatEntry(output,    x - a, (x-b)^2/(b-a));
            updateQMNatEntry(output, -(x - b), (x-a)^2/(b-a));
        else
            return case_3_1(b, a);
        end if;
        return output;
    ...
    local output := zeroQM(nat);
        updateQMNatEntry(output,       (x-a), (x-b)^2);
        updateQMNatEntry(output, (x-a)*(x-b),   (b-a));
        return output;
    end proc;
    ...
    local output := zeroQM(nat);
    local alpha := (b-a)/(c-b);
        updateQMNatEntry(output, (x-a)*(x-b), alpha/(alpha+1)*(x-c)^2);
        updateQMNatEntry(output, (x-b)*(x-c),     1/(alpha+1)*(x-a)^2);
        return output;
    end proc;
    ...
    local output := zeroQM(nat), _temp1, _temp2;
        DEBUG(__FILE__, __LINE__, ENABLE_DEBUGGING, lprint(">> certPO"));
        DEBUG(__FILE__, __LINE__, ENABLE_DEBUGGING, print(certPO));
        ...
        for basis in [indices(certPO, 'nolist')] do
            if evalb(certPO[basis] = 0) = false then
                ...
                _temp1 := unitQM(nat);
                i := 0;
                for gen in Split(basis, bits=nops(nat)) do
                    #DEBUG(__FILE__, __LINE__, ENABLE_DEBUGGING, lprint(">> gen@certInQM", nat[gen]));
                    i := i + 1;
                    if gen = 0 then
                      next;
                    end if;
                    _temp2 := zeroQM(nat);
                    updateQMNatEntry(_temp2, nat[i], 1);
                    st := time();
                    DEBUG(__FILE__, __LINE__, ENABLE_DEBUGGING, lprint(">> Done 1 one step within gen@certInQM", fromQMtoPoly(_temp1)));
                    DEBUG(__FILE__, __LINE__, ENABLE_DEBUGGING, lprint(">> Done 2 one step within gen@certInQM", fromQMtoPoly(_temp2)));
                    _temp1 := prodQM(_temp1, _temp2, a_0, b_k, nat, x);
                    DEBUG(__FILE__, __LINE__, ENABLE_DEBUGGING, lprint(">> Done 3 one step within gen@certInQM", fromQMtoPoly(_temp1)));
                end do;
